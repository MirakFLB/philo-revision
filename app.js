/* =====================================================================
   Philosophie Terminale — Révision Bac 2026 · Application
   Routing hash · progression & gamification (localStorage) · 10 jeux.
   Feature signature : tri des auteurs par couleur 🔵🔴🟣 → plan I/II/III.
   ===================================================================== */
(function () {
  "use strict";

  const DATA = window.PHILO_DATA, CONTENT = window.PHILO_CONTENT || {}, QUIZ = window.PHILO_QUIZ || {};
  const PARTS = DATA.parts;
  const orderedNotions = PARTS.flatMap(p => p.notions);
  const partOf = {}, notionById = {};
  PARTS.forEach(p => p.notions.forEach(c => { partOf[c.id] = p; notionById[c.id] = c; }));
  const COULEURS = {}; DATA.couleurs.forEach(c => COULEURS[c.id] = c);
  ...
  const LEVELS = [
    { min: 0,    name: "Lycéen" },
    { min: 120,  name: "Apprenti penseur" },
    { min: 350,  name: "Dialecticien" },
    { min: 700,  name: "Philosophe" },
    { min: 1200, name: "Esprit libre" },
  ...
  function cardsMastered(){ let n = 0; for (const k in state.leitner) if (state.leitner[k] >= 4) n++; return n; }
  function partAllDone(pid){ const p = PARTS.find(x => x.id===pid); return p.notions.every(c => c.objectifs.every((o,i)=>state.done.has(c.id+"#o"+i))); }
  function allDone(){ return orderedNotions.every(c => c.objectifs.every((o,i)=>state.done.has(c.id+"#o"+i))); }
  ...
  const BADGES = [
    ...
    { id:"memory",    icon:"🧠", name:"Par cœur", desc:"20 concepts maîtrisés en cartes" },
    { id:"t1master",  icon:"🥇", name:"Tier 1 bouclé", desc:"Tous les objectifs du Tier 1 maîtrisés" },
    { id:"exam",      icon:"📜", name:"Copie solide", desc:"Au moins 10/12 à l'examen blanc" },
    { id:"ready",     icon:"🎓", name:"Prêt pour le Bac", desc:"Tous les objectifs des 17 notions maîtrisés" }
  ];
  function checkBadges(extra){
    ...
    earn("memory", cardsMastered() >= 20);
    earn("t1master", partAllDone("t1"));
    earn("exam", (state.best.exam||0) >= 10);
    earn("ready", allDone());
  ...
  const TITLES = [
    { id:"stoicien",     name:"Le Stoïcien",        price:200 },
    { id:"socratique",   name:"Esprit socratique",  price:300 },
    { id:"prepa",        name:"Futur Prépa",        price:400 },
    { id:"dialecticien", name:"Maître dialectique", price:500 },
  ...
  const badgeTier = c => `<span class="badge dot ${c.tier===1?"ok":c.tier===2?"":"oral"}">${tierLabel(c.tier)}</span>`;
  ...
  function gamificationStrip(){
    ...
    return `<div class="gstrip">
      ...
      <a class="gstat go" href="#/jouer"><span class="gk">Jouer</span><span class="gv">10 modes →</span><span class="gsub">QCM, récitation, cartes, examen…</span></a>
    </div>`;
  }
  ...
  function renderDashboard(){
    ...
    const partBars = PARTS.map(p => { const pp = partProgress(p);
      return `<a class="card prow" href="#/programme" data-accent="${p.accent}">${ring(pp.pct,52)}
        <div style="flex:1"><div class="prow-t">${esc(p.name)}</div><div class="muted prow-s">${p.notions.length} notion${p.notions.length>1?"s":""} · ${pp.d}/${pp.t} objectifs</div></div>
    ...
    const tierList = PARTS.map(p => `<li><span class="tdot ${p.accent}"></span><span><b>${esc(p.name)}</b><br><span class="field">${esc(p.desc)}</span><br><span class="field">${p.notions.map(n=>esc(n.titre)).join(" · ")}</span></span></li>`).join("");
    ...
    return `<div class="view fade" data-accent="gold">
      <section class="hero">
        ...
        <h1 class="display">Réviser toute la philo, sans impasse.</h1>
        <p class="lead">${esc(m.session)} — 17 notions, le code couleur 🔵🔴🟣 pour bâtir tes plans, la méthode des deux épreuves, et 10 jeux pour ancrer le cours. Récite, joue, gagne de l'XP, suis ta progression.</p>
        <div class="facts">
          ...
          <div class="fact"><div class="k">17</div><div class="l">Notions au programme</div></div>
          <div class="fact"><div class="k" style="font-size:18px;line-height:1.2">${esc(m.dateEcrit)}</div><div class="l">Épreuve écrite</div></div>
        </div>
      </section>
      ...
      <div class="toolbar" style="margin-top:18px">
        ...
        <a class="btn ghost" href="#/programme">Parcourir les notions</a>
      </div>

      <h2 class="section">La stratégie par tiers</h2>
      <p class="lead" style="font-size:15px">${esc(m.note)}</p>
      <div class="callout" style="margin-top:16px"><ul class="tier-list">${tierList}</ul></div>
    </div>`;
  }

  function renderProgramme(){
    const blocks = PARTS.map(p => { const pp = partProgress(p);
      const notions = p.notions.filter(c => !state.prioOnly || c.tier<=2).map(c => { const cp = notionProgress(c);
        return `<a class="card chap" href="#/chapitre/${c.id}" data-accent="${p.accent}">
          <div class="num">Notion ${c.num}</div><div class="title">${esc(c.titre)}</div>
          <div class="meta">${badgeTier(c)}<span class="badge">${c.objectifs.length} obj.</span>${ring(cp.pct,44)}</div></a>`; }).join("");
      if (!notions) return "";
      return `<section class="part" data-accent="${p.accent}"><div class="part-head"><span class="part-dot"></span><h2>${esc(p.name)}</h2><span class="count">${pp.pct}% · ${p.notions.length} notion${p.notions.length>1?"s":""}</span></div><div class="grid cols-2">${notions}</div></section>`; }).join("");
    return `<div class="view fade" data-accent="gold">
      <p class="eyebrow">Programme</p><h1 class="display">Les 17 notions</h1>
      <p class="lead">Clique sur une notion : le cœur (idée + problème), les distinctions, le tri des auteurs par couleur → plan, et le par cœur (citations, accroches, pièges).</p>
      <div class="toolbar" style="margin-top:18px"><label class="toggle"><input type="checkbox" id="prioOnly" ${state.prioOnly?"checked":""}><span class="sw"></span> Masquer le Tier 3 (fiches express anti-impasse)</label></div>
      ${blocks}</div>`;
  }
  ...
  function onStabiloSelect(){
    ...
    view.innerHTML = renderNotion(id);
  }
  function removeHLAt(id, p, rs, re){
    ...
    save(); view.innerHTML = renderNotion(id);
  }
  ...
  function renderNotion(id){
    ...
    if (chapTab === "coeur"){
      ...
      const concepts = (ct.notions||[]).map(n=>`<div class="notion"><div class="t">${esc(n.t)}</div><div class="d">${esc(n.d)}</div></div>`).join("");
      body = `<div class="obj-head" style="margin-top:6px"><h3 class="sub3">Distinctions à ne pas confondre</h3><span class="aide-tag">les couples-clés</span></div>
        <ul class="dist-list">${dist||"<li class='muted'>—</li>"}</ul>
        <h3 class="sub3" style="margin-top:24px">Concepts à connaître</h3><div class="notion-grid">${concepts||"<p class='muted'>—</p>"}</div>`;

    } else if (chapTab === "couleur"){
      const C2 = ct.couleur || {};
      const colBlock = (key) => { const col = C2[key]; if(!col) return ""; const meta = COULEURS[key];
        const auths = (col.auteurs||[]).map(a=>`<li><b>${esc(a.n)}</b> — ${esc(a.d)}</li>`).join("");
        return `<div class="tri-col ${key}"><div class="tri-head">${meta.emoji} ${esc(meta.camp)}</div><div class="tri-sub">${esc(col.titre||"")}</div><ul class="tri-auth">${auths}</ul></div>`; };
      const cols = ["bleu","rouge","violet"].map(colBlock).join("");
      const plans = (C2.plans||[]).map(pl=>`<div class="plan-card">
        <div class="plan-sujet">« ${esc(pl.sujet)} »</div>
        <div class="plan-row"><span class="plan-num bleu">I</span><span>${esc(pl.I)}</span></div>
        <div class="plan-row"><span class="plan-num rouge">II</span><span>${esc(pl.II)}</span></div>
        <div class="plan-row"><span class="plan-num violet">III</span><span>${esc(pl.III)}</span></div></div>`).join("");
      body = `<div class="obj-head" style="margin-top:6px"><h3 class="sub3">Tri des auteurs par couleur</h3><span class="aide-tag">🔵 thèse · 🔴 antithèse · 🟣 dépassement</span></div>
        <div class="tri-grid">${cols}</div>
        ${plans?`<h3 class="sub3" style="margin-top:26px">Plan(s)-type(s)</h3><div class="plan-wrap">${plans}</div>`:`<p class="muted" style="margin-top:18px">Construis ton plan : I (🔵) → II (🔴) → III (🟣 : conteste les présupposés, dégage un paradoxe).</p>`}`;

    } else if (chapTab === "parcoeur"){
      const cites = (ct.parcoeur||[]).map(s=>`<li>${esc(s)}</li>`).join("");
      const accs = (ct.accroches||[]).map(s=>`<li>${esc(s)}</li>`).join("");
      const piegs = (ct.pieges||[]).map(s=>`<li>${esc(s)}</li>`).join("");
      body = `<div class="obj-head" style="margin-top:6px"><h3 class="sub3">Citations &amp; formules-chocs</h3><span class="aide-tag">à placer en copie</span></div>
        <ul class="cite-list">${cites||"<li class='muted'>—</li>"}</ul>
        <h3 class="sub3" style="margin-top:24px">Accroches (planter un décor)</h3><ul class="acc-list">${accs||"<li class='muted'>—</li>"}</ul>
    ...
    return `<div class="view fade" data-accent="${p.accent}">
      <a class="back" href="#/programme"><svg viewBox="0 0 24 24"><path d="M15 6l-6 6 6 6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> Les 17 notions</a>
      <div class="detail-top"><div class="detail-head" style="flex:1;min-width:260px"><p class="eyebrow">${esc(p.name)} · Notion ${c.num}</p><h1>${esc(c.titre)}</h1>
        <div class="chap-tags">${badgeTier(c)}<span class="badge">${c.objectifs.length} objectifs</span><span class="badge">${(ct.notions||[]).length} concepts</span></div></div>
        <div style="display:grid;place-items:center">${ring(cp.pct,132)}</div></div>
      <div class="tabs">${tabBtns}</div>
      <div class="tab-body">${body}</div>
      <div class="toolbar" style="margin-top:30px"><button class="btn" data-act="revise-chap" data-id="${c.id}"><svg viewBox="0 0 24 24" width="18" height="18"><path d="M5 3l14 9-14 9z" fill="currentColor"/></svg> Jouer sur cette notion</button></div>
      <nav class="chap-nav">${navLink(prev,"prev")}${navLink(next,"next")}</nav></div>`;
  }

  function renderMethode(){
    const M = DATA.methode;
    const reflexes = M.dissertation.reflexes.map(r=>`<div class="epr-part"><div class="pt-body"><strong>${esc(r.t)}</strong><p>${esc(r.d)}</p></div></div>`).join("");
    const plan = M.dissertation.plan.map(r=>`<div class="epr-part"><div class="pt-body"><strong>${esc(r.t)}</strong><p>${esc(r.d)}</p></div></div>`).join("");
    const regles = M.dissertation.reglesOr.map(s=>`<li><svg viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg><span>${esc(s)}</span></li>`).join("");
    const explic = M.explication.points.map(s=>`<li><svg viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg><span>${esc(s)}</span></li>`).join("");
    const jourJ = M.jourJ.map(s=>`<li><svg viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg><span>${esc(s)}</span></li>`).join("");
    return `<div class="view fade" data-accent="gold">
      <p class="eyebrow">Épreuve du Bac</p><h1 class="display">Méthode des épreuves</h1><p class="lead">${esc(M.intro)}</p>
      <h2 class="section">Sujet A — La dissertation</h2>
      <div class="card method-card"><h3>Les 3 réflexes sur l'intitulé <span class="aide-tag">au brouillon</span></h3><div class="parts-epreuve">${reflexes}</div></div>
      <div class="card method-card" data-accent="eco" style="margin-top:16px"><h3>Le plan I / II / III</h3><div class="parts-epreuve">${plan}</div></div>
      <div class="card method-card" style="margin-top:16px"><h3>Règles d'or</h3><ul class="attendus">${regles}</ul></div>
      <h2 class="section">Sujet B — L'explication de texte</h2>
      <div class="card method-card"><h3>${esc(M.explication.titre)}</h3><ul class="attendus">${explic}</ul></div>
      <h2 class="section">Stratégie jour J</h2>
      <div class="card method-card" data-accent="socio"><ul class="attendus">${jourJ}</ul></div>
    </div>`;
  }

  function renderTransversaux(){
    const legend = DATA.couleurs.map(co=>`<div class="cc-card ${co.id}"><div class="cc-head">${co.emoji} ${esc(co.camp)}</div><div class="cc-idea">${esc(co.idee)}</div><div class="cc-role">${esc(co.role)}</div></div>`).join("");
    const authors = DATA.transversaux.map(a=>`<div class="notion auth tcard ${a.couleur}"><div class="t">${esc(a.n)} ${COULEURS[a.couleur]?COULEURS[a.couleur].emoji:""}</div><div class="d">${esc(a.these)}</div><div class="tmeta"><span class="tnot">Notions : ${esc(a.notions)}</span><span class="tform">${esc(a.formule)}</span></div></div>`).join("");
    return `<div class="view fade" data-accent="gold">
      <p class="eyebrow">Code couleur &amp; auteurs</p><h1 class="display">Auteurs &amp; couleurs</h1>
      <p class="lead">La clé de voûte des plans : range chaque auteur en 🔵 / 🔴 / 🟣, puis pose-les sur un plan I / II / III.</p>
      <div class="cc-grid">${legend}</div>
      <h2 class="section">12 auteurs transversaux (à recaser partout)</h2>
      <div class="notion-grid">${authors}</div>
    </div>`;
  }

  /* ===================== GAME HUB ===================== */
  const MODES = [
    { id:"recall", icon:"🧠", name:"Récitation", desc:"Question → tu reconstruis la réponse de tête, puis tu révèles." },
    ...
    { id:"flash", icon:"🃏", name:"Cartes mémoire", desc:"Concept → définition, répétition espacée." },
    { id:"match", icon:"🔗", name:"Associations", desc:"Relie chaque concept à sa définition." },
    { id:"trous", icon:"✏️", name:"Textes à trous", desc:"Complète le mot manquant." },
    { id:"exam",  icon:"⏱️", name:"Examen blanc", desc:"Mix chronométré sur le Tier 1, score final." },
    { id:"objectifs", icon:"✅", name:"Objectifs (auto-éval)", desc:"Sais-je traiter chaque objectif de révision ?" },
  ...
  const hub = { mode:"recall", scope:"t1" };

  function notionsInScope(scope){
    if (scope==="all") return orderedNotions;
    if (scope==="t1"||scope==="t2"||scope==="t3"){ const p=PARTS.find(x=>x.id===scope); return p?p.notions:orderedNotions; }
    if (notionById[scope]) return [notionById[scope]];
    return orderedNotions;
  }
  function scopeLabel(scope){ if(scope==="all")return"Toutes les notions"; const p=PARTS.find(x=>x.id===scope); if(p)return p.name; if(notionById[scope])return notionById[scope].titre; return "Sélection"; }
  function countFor(mode, scope){
    const chs = notionsInScope(scope); let n = 0;
    chs.forEach(c=>{ if(mode==="flash"||mode==="match") n+=(CONTENT[c.id]&&CONTENT[c.id].notions||[]).length;
      else if(mode==="objectifs") n+=c.objectifs.length;
      else if(mode==="recall") n+=(QUIZ[c.id]&&QUIZ[c.id].recall||[]).length;
  ...
  function renderHub(){
    ...
    const scopes = [["t1","Tier 1 — priorité"],["all","Toutes les notions"],["t2","Tier 2"],["t3","Tier 3"]];
    let chips = scopes.map(s=>`<button class="chip ${hub.scope===s[0]?"sel":""}" data-act="hub-scope" data-scope="${s[0]}">${esc(s[1])}</button>`).join("");
    if (notionById[hub.scope]) chips += `<button class="chip sel" data-act="hub-scope" data-scope="${hub.scope}">${esc(notionById[hub.scope].titre)}</button>`;
    ...
    return `<div class="view fade" data-accent="gold">
      ...
      <p class="lead">10 façons d'apprendre. La <b>Récitation</b> est reine en philo : tu reconstruis la réponse de tête. Chaque bonne réponse rapporte de l'XP, fait monter ton niveau et entretient ta série.</p>
      ${gamificationStrip()}
      ...
      ${ok?"":`<p class="muted" style="margin-top:10px">Pas assez d'éléments pour ce mode sur cette sélection — choisis « Toutes les notions ».</p>`}
    </div>`;
  }
  ...
  function buildSession(mode, scope){
    const chs = notionsInScope(scope);
    ...
    const collectAll = kind => { const out=[]; orderedNotions.forEach(c=>{ (QUIZ[c.id]&&QUIZ[c.id][kind]||[]).forEach(d=>out.push({kind, d, c})); }); return dedupe(out); };
    if (mode==="qcm")  s.items = shuffle(collect("qcm")).slice(0,12);
    ...
    else if (mode==="recall"){ s.items = shuffle(collect("recall")).slice(0,12); }
    else if (mode==="exam"){
      const epool = orderedNotions.filter(c=>c.tier===1);
      const q=[]; epool.forEach(c=>(QUIZ[c.id]&&QUIZ[c.id].qcm||[]).forEach(d=>q.push({kind:"qcm",d,c})));
      const v=[]; epool.forEach(c=>(QUIZ[c.id]&&QUIZ[c.id].vf||[]).forEach(d=>v.push({kind:"vf",d,c})));
      ...
      const obs=[]; chs.forEach(c=>c.objectifs.forEach((o,idx)=>obs.push({kind:"objectif", d:{text:o}, c, oid:c.id+"#o"+idx})));
      s.items = shuffle(obs);
    } else if (mode==="sprint" || mode==="survie"){
      // Modes arcade : on tire du banc COMPLET dédoublonné (jamais de répétition dans une partie).
      // Si la notion choisie offre déjà assez de variété (>=25 QCM distincts), on la respecte ; sinon on élargit à tout.
  ...
  function buildMatchRound(s, chs){
    chs = chs || notionsInScope(s.scope);
  ...
  function renderGame(){
    ...
    let card = "";
    if (it.kind==="qcm") card = renderQCM(it);
    ...
    else if (it.kind==="recall") card = renderRecall(it);
  ...
  function renderQCM(it){
    ...
    return `<div class="q-card"><div class="q-chap">${esc(it.c.titre)}</div><div class="q-text">${esc(q.q)}</div><div class="opts">${opts}</div>${feedback}<div class="q-actions">${actions}</div></div>`;
  }
  function renderVF(it){
    ...
    return `<div class="q-card"><div class="q-chap">${esc(it.c.titre)}</div><div class="q-text">${esc(q.s)}</div><div class="vf-row">${btn(true,"Vrai","yes")}${btn(false,"Faux","no")}</div>${feedback}<div class="q-actions">${nextBtn}</div></div>`;
  }
  function renderTrou(it){
    ...
    return `<div class="q-card"><div class="q-chap">${esc(it.c.titre)}</div><div class="q-text trou">${esc(parts[0])}${filled}${esc(parts[1]||"")}</div>${feedback}<div class="q-actions">${actions}</div></div>`;
  }
  function renderFlash(it){
    ...
    return `<div class="flash ${g.flipped?"flip":""}" data-act="flip"><div class="flash-inner">
      <div class="flash-face flash-front"><div class="tagline">Concept · boîte ${box}/5</div><div class="q">${esc(n.t)}</div><div class="hint">Clique pour révéler</div><div class="ctx">${esc(it.c.titre)}</div></div>
      <div class="flash-face flash-back"><div class="tagline">Définition</div><div class="a small">${esc(n.d)}</div></div></div>
  ...
  function renderRecall(it){
    const q = it.d;
    const revealed = g.flipped;
    const answer = revealed ? `<div class="recall-answer"><div class="tagline">Réponse modèle</div><p>${esc(q.a)}</p></div>` : `<button class="btn ghost reveal-btn" data-act="flip">Révéler la réponse</button>`;
    const rate = revealed ? `<div class="player-actions"><button class="btn again" data-act="recall-rate" data-v="0">Pas su</button><button class="btn know" data-act="recall-rate" data-v="1">Je l'avais ✓</button></div>` : "";
    return `<div class="q-card recall"><div class="q-chap">${esc(it.c.titre)}</div><div class="q-text" style="font-size:19px">${esc(q.q)}</div>
      <p class="muted" style="margin:10px 0 16px">Reconstruis la réponse de tête, à voix haute, puis révèle.</p>
      ${answer}${rate}
      ${revealed?'<p class="flash-reveal-hint" style="margin-top:14px">← Pas su · → Je l\'avais</p>':'<p class="flash-reveal-hint" style="margin-top:14px">Espace : révéler</p>'}</div>`;
  }
  function renderObjectif(it){
    return `<div class="q-card"><div class="q-chap">Objectif de révision · ${esc(it.c.titre)}</div><div class="q-text" style="font-size:18px">${esc(it.d.text)}</div>
      <p class="muted" style="margin:14px 0 0">Sais-tu traiter cet objectif (définir, expliquer, illustrer) ?</p>
  ...
  function renderMatch(){
    ...
    return `<div class="view fade" data-accent="gold"><div class="player wide">${gameTop("gold")}
      <p class="match-help">Clique un concept, puis sa définition.</p>
  ...
  function cqHTML(a){
    let ans;
    if (a.kind==="recall") ans = `<span class="cq-good">Réponse : ${esc(a.corr)}</span>`;
    else if (a.ok) ans = `<span class="cq-good">Bonne réponse : ${esc(a.corr)}</span>`;
  ...
  function renderResult(){
    ...
    if (!g.rewarded){
      ...
      else if (g.sprint){ g._isBest = g.correct > (state.best.sprint||0); if (g._isBest) state.best.sprint = g.correct; }
      else if (g.survie){ g._isBest = g.correct > (state.best.survie||0); if (g._isBest) state.best.survie = g.correct; }
    ...
    } else if (g.mode==="recall"){ titleTxt = "Récitation terminée"; extra = `<p>${g.correct} réponse${g.correct>1?"s":""} sue${g.correct>1?"s":""} sur ${total}.</p>`;
    } else {
  ...
  function recallRate(know){
    const it=g.items[g.i];
    g.answers.push({ chap:reviewLabel(it), kind:"recall", q:it.d.q, your:"", corr:it.d.a, ok:know, e:"", note:courseNote(it.c.id) });
    if (know){ g.correct++; addXp(10); gainCoins(5); sfx("correct"); creditObjective(it.c.id, it.c.id+"|recall|"+(it.d.q||"")); }
  ...
  function renderProfil(){
    ...
    return `<div class="view fade" data-accent="gold">
      ...
      <div class="stat-grid">
        ...
        <div class="stat-c"><div class="sc-k">${cardsMastered()}</div><div class="sc-l">Concepts maîtrisés</div></div>
  ...
  function route(){
    ...
    switch(seg[0]){
      ...
      case "chapitre": html=renderNotion(seg[1]); active="/programme"; break;
      case "methode": html=renderMethode(); active="/methode"; break;
      case "outils": html=renderTransversaux(); active="/outils"; break;
  ...
  document.addEventListener("click", function(e){
    ...
    switch(act){
      case "toggle-obj": { const id=el.getAttribute("data-id"); if(state.done.has(id))state.done.delete(id);else state.done.add(id); const nb=checkBadges(); save(); announceBadges(nb); el.classList.toggle("done"); const seg=(location.hash||"").replace(/^#/,"").split("/").filter(Boolean); if(seg[0]==="chapitre") softRefreshNotion(seg[1]); renderSidebar(); break; }
      case "stabilo": { state.stabilo=!state.stabilo; save(); const seg=(location.hash||"").replace(/^#/,"").split("/").filter(Boolean); if(seg[0]==="chapitre") view.innerHTML=renderNotion(seg[1]); break; }
      case "hl-clear": { const seg=(location.hash||"").replace(/^#/,"").split("/").filter(Boolean); if(seg[0]==="chapitre"){ state.hl[seg[1]]=[]; save(); view.innerHTML=renderNotion(seg[1]); } break; }
      case "tab": chapTab=el.getAttribute("data-tab"); { const seg=(location.hash||"").replace(/^#/,"").split("/").filter(Boolean); view.innerHTML=renderNotion(seg[1]); } break;
      ...
      case "revise-chap": hub.scope=el.getAttribute("data-id"); hub.mode="recall"; g=null; location.hash="#/jouer"; break;
      ...
      case "flip": if(g&&g.items[g.i]){ const k=g.items[g.i].kind; if(k==="flash"){ g.flipped=!g.flipped; const f=view.querySelector(".flash"); if(f) f.classList.toggle("flip"); } else if(k==="recall"){ g.flipped=true; view.innerHTML=renderGame(); } } break;
      case "flash-rate": flashRate(el.getAttribute("data-v")==="1"); break;
      case "recall-rate": recallRate(el.getAttribute("data-v")==="1"); break;
  ...
  document.addEventListener("change", function(e){ if(e.target&&e.target.id==="prioOnly"){ state.prioOnly=e.target.checked; save(); view.innerHTML=renderProgramme(); } else if(e.target&&e.target.id==="soundToggle"){ state.sound=e.target.checked; save(); if(state.sound){ ensureAudio(); sfx("coin"); } } });
  ...
  /* ===================== SEARCH ===================== */
  const SEARCH_INDEX = [];
  orderedNotions.forEach(c=>{
    SEARCH_INDEX.push({type:"Notion",title:c.titre,ctx:`Notion ${c.num} · ${partOf[c.id].name}`,href:`#/chapitre/${c.id}`,hay:c.titre.toLowerCase()});
    c.objectifs.forEach(o=>{ SEARCH_INDEX.push({type:"Objectif",title:o,ctx:`${c.titre}`,href:`#/chapitre/${c.id}`,hay:o.toLowerCase()}); });
    const ct = CONTENT[c.id]||{};
    (ct.notions||[]).forEach(n=>SEARCH_INDEX.push({type:"Concept",title:n.t,ctx:n.d,href:`#/chapitre/${c.id}`,hay:(n.t+" "+n.d).toLowerCase()}));
    ["bleu","rouge","violet"].forEach(k=>{ const col=ct.couleur&&ct.couleur[k]; if(col) (col.auteurs||[]).forEach(a=>SEARCH_INDEX.push({type:"Auteur",title:a.n,ctx:`${c.titre} — ${a.d}`,href:`#/chapitre/${c.id}`,hay:(a.n+" "+a.d+" "+c.titre).toLowerCase()})); });
    (ct.parcoeur||[]).forEach(s=>SEARCH_INDEX.push({type:"Citation",title:s,ctx:c.titre,href:`#/chapitre/${c.id}`,hay:s.toLowerCase()}));
  });
  DATA.transversaux.forEach(a=>SEARCH_INDEX.push({type:"Auteur",title:a.n,ctx:a.these,href:`#/outils`,hay:(a.n+" "+a.these+" "+a.notions).toLowerCase()}));
  ...
  function runSearch(q){
    ...
    const order={Notion:0,Auteur:1,Concept:2,Citation:3,Objectif:4};
    ...
    let html="";
    ["Notion","Auteur","Concept","Citation","Objectif"].forEach(grp=>{ if(!groups[grp])return; const labels={Notion:"Notions",Auteur:"Auteurs",Concept:"Concepts",Citation:"Citations",Objectif:"Objectifs"}; html+=`<div class="sr-group-title">${labels[grp]}</div>`;
  ...
  document.addEventListener("keydown",function(e){
    ...
    if(g && location.hash.indexOf("jouer")>=0 && g.i<g.items.length){
      ...
      else if(it.kind==="recall"){ if(!g.flipped){ if(e.key===" "){e.preventDefault(); g.flipped=true; view.innerHTML=renderGame();} } else { if(e.key==="ArrowRight")recallRate(true); else if(e.key==="ArrowLeft")recallRate(false); } }
